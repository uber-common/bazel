...
class DirectoryTreeBuilder {
    ...
    return build(
        ...
        (input, path, currDir) -> {
          if (input instanceof VirtualActionInput) {
            ...
            if (spawnScrubber != null && spawnScrubber.shouldScrubParamFiles() && virtualActionInput instanceof ParamFileActionInput) {
  ...
  private static <T> int build(
    ...
    for (Map.Entry<PathFragment, T> e : inputs.entrySet()) {
      ...
      if (scrubber != null && scrubber.shouldOmitInput(path, spawn)) {
        continue;
      }
