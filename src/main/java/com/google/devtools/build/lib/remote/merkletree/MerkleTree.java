...
public class MerkleTree {
    ...
    try (SilentCloseable c = Profiler.instance().profile("MerkleTree.build(ActionInput)")) {
      DirectoryTree tree =
          DirectoryTreeBuilder.fromActionInputs(
              inputs, metadataProvider, execRoot, artifactPathResolver, spawnScrubber, digestUtil);
  ...
  public static MerkleTree build(
    ...
    try (SilentCloseable c = Profiler.instance().profile("MerkleTree.build(ActionInput)")) {
      DirectoryTree tree =
          DirectoryTreeBuilder.fromActionInputs(
              inputs, toolInputs, metadataProvider, execRoot, artifactPathResolver, spawnScrubber, digestUtil);
      return build(tree, digestUtil);
    }
  }
